					...
					if (!$(this).hasClass('copy-lead__button_disable')) {
						let pipeline_id = Number($('.copy-lead [name="select-pipeline"]').val()),
							...
							success = $.ajax({
								...
								dataType: 'json',
								success: function (data) {
									let contact_id = data._embedded['contacts'].map(item => item.id);
			                        $.ajax({
								    	method: 'POST',
										url: '/api/v4/leads',
										dataType: 'json',
										data: JSON.stringify([{
											...
											custom_fields_values: data.custom_fields_values.map(item => { return { field_id: item.field_id, values: item.values } }) 
										}]),
										success: function (data) {
											$.ajax({
												method: 'POST',
												...
												data: JSON.stringify([
													{
														"to_entity_id": Number(contact_id),
